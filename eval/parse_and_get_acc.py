...
def display_results(setups):
    """
    Display the results in a formatted table using the rich library.

    Args:
        setups (dict): A dictionary containing setup information and results.
    """
    ...
    table.add_column("📏 Gen Length", justify="center", style="blue")
    table.add_column("📈 Diffusion Steps", justify="right", style="cyan")
    table.add_column("🎯 Accuracy (▲)", justify="center", style="red")
    table.add_column("#Samples", justify="center", style="bold")
    table.add_column("Avg Effective Tokens", justify="right", style="bold")

    # Calculate final accuracy and save results
    for setup, results in sorted(setups.items()):
        # Extract metadata from setup string
        ...
        if metadata:
            ...
            checkpoint = metadata.get("checkpoint", "N/A")
            gen_length = metadata.get("gen_length", "N/A")
            diffusion_steps = metadata.get("diffusion_steps", "N/A")
        else:
            parts = setup.split('_')
            dataset = parts[0]
            model_name = parts[1]
        ...
        num_samples = len(results["questions"])
        ...
        table.add_row(
            ...
            f"{num_samples}",
            f"{results['avg_effective_tokens']:.2f}"
        )
...
def extract_setup_name(filename):
    ...
    # f"{args.dataset}_{model_name}_{args.gen_length}_{args.diffusion_steps}_{dist.get_rank()}_generations.json"
    if match:
...
def extract_filename_components(filename):
    """
    Extracts dataset, model_name, gen_length, and diffusion_steps from a given filename.

    Args:
        filename (str): The filename to extract components from.

    Returns:
        dict: A dictionary containing the extracted components, or None if no match is found.
    """
    # Regular expression pattern to match the components
    baseline = False if "checkpoint" in filename else True
    if baseline:
        pattern = r"(?P<dataset>[^_/]+)_(?P<model_name>[^_]+)_(?P<gen_length>\d+)_(?P<diffusion_steps>\d+)_\d+_generations\.json$"
    else:
        pattern = r"(?P<dataset>[^_/]+)_(?P<model_name>[^_]+)_(?P<checkpoint>[^_]+)_(?P<gen_length>\d+)_(?P<diffusion_steps>\d+)_\d+_generations\.json$"

    # Use re.search to find the components in the filename
    ...
    if match:
        ...
        if baseline:
            components = {
                ...
                'gen_length': match.group('gen_length'),
                'diffusion_steps': match.group('diffusion_steps')
            }
        else:
            components = {
                'dataset': match.group('dataset'),
...
def aggregate_results(directory="."):
    ...
    for json_file in json_files:
        ...
        metadata = extract_filename_components(filename)
        ...
        if setup_name:
            ...
            setups[setup_name]["metadata"] = metadata # rewriting metadata from different ranks is fine
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Directory to aggregate results from")
    args = parser.parse_args()

    setups = aggregate_results(directory=args.directory)
    display_results(setups)
